    ...
    <div className="p-6 flex flex-wrap justify-between border-b border-blueGray-800">
      <div className="w-2/4">
        <div className="flex flex-col h-full">
          <h6 className="font-bold text-white mb-1">{data.name}</h6>
          <span className="block pb-4 mb-auto font-medium text-gray-400">
            {data.quantity} x ${centsToDollars(data.price)}
          </span>
        </div>
      </div>
      <div className="w-1/4">
        <div className="flex flex-col items-end h-full">
          ...
          <span className="block mt-2 text-sm font-bold text-white">
            ${centsToDollars(data.price * data.quantity)}
          </span>
        </div>
      </div>
    </div>
    ...
    <section className="ml-4">
      <div className="relative">
        <button
          ...
          className=" bg-yellow-500 text-white p-3 rounded-full hover:bg-yellow-500 items-center"
        >
          <svg
            width="24"
            height="24"
        ...
        {showCart && (
          <div className="absolute right-0 top-16 z-10 w-[300px] md:w-[400px] rounded-3xl co bg-gray-800">
